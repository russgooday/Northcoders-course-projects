...
from .tables_classes import (Shops, Treasures)
...
# This is the main function to seed the database
# CHANGED: No longer drops and creates tables, just seeds the existing tables
def seed_db(session: Session, env: str='dev') -> tuple[int, int] | None:
    '''Seed the database using SQLAlchemy ORM
        Args:
            session (Session): the SQLAlchemy session to use for database operations
            env (str): the environment 'dev' or 'prod' to seed the database with
        Returns:
    ...
    shops = session.execute(
        insert(Shops)
            .values(shops_df.to_dict('records'))
            .on_conflict_do_nothing(index_elements=['shop_id'])
    )

    treasures = session.execute(
        insert(Treasures)
            .values(treasures_df.to_dict('records'))
            .on_conflict_do_nothing(index_elements=['treasure_id'])
    )

    print(f'Database seeded! Inserted {treasures.rowcount} treasures.')
